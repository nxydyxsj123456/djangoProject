...
class RainYun():
    ...
    def __init__(self, user: str, pwd: str) -> None:
        ...
        self.sendSms_url = "https://api-recruitment.yzw.cn/v2/labor/app/sendSms"
        self.smsLogin_url = "https://api-recruitment.yzw.cn/v2/labor/app/smsLogin"
        self.logout_url = "https://api.v2.rainyun.com/user/logout"
        self.query_url = "https://api.v2.rainyun.com/user/"
        self.choujiang_url="https://api.v2.rainyun.com/user/reward/items"
        # 忽略 .cc ssl错误
    ...
    def login(self) -> None:
        ...
        res = self.session.post(
            url=self.login_url, headers={
                ...
                "appsourcetype": "1",
                "accept-encoding": "gzip",
                "cookie": "HWWAFSESTIME="+int(round(time.time() * 1000))+"; HWWAFSESID=4bc2b7c8808792ec53",
                                         }, data=self.json_data)
    ...
    def sendSms(self) -> None:
        """签到"""
        res = self.session.post(url=self.sendSms_url, headers={
                "Content-Type": "application/json",
                "host": "api-recruitment.yzw.cn",
                "content-length": "29",
                "accept": "application/json, text/plain, */*",
                ...
                "appsourcetype": "1",
                "accept-encoding": "gzip",
                "cookie": "HWWAFSESTIME="+str(int(round(time.time() * 1000)))+"; HWWAFSESID=4bc2b7c8808792ec53",
                                         }, data=json.dumps({
            "phoneNumber": "15869171029"
        }))


        self.signin_date = datetime.utcnow()
        if res.text.find("20000") > -1:
            self.logger.info("成功签到并领取积分")
            self.signin_result = True
        else:
            self.logger.error(f"签到失败，响应信息：{res.text}")
            self.signin_result = False
    def smsLogin(self,phone,msg) -> None:
        """签到"""
        res = self.session.post(url=self.smsLogin_url, headers={
                "Content-Type": "application/json",
                "host": "api-recruitment.yzw.cn",
                # "content-length": "29",
                "accept": "application/json, text/plain, */*",
        ...
        self.signin_date = datetime.utcnow()
        if res.text.find("20000") > -1:
            self.logger.info(f"成功签到并领取积分{res.text}")
            parsed_json = json.loads(res.text)
            self.session_token = parsed_json["data"]["sessionToken"]
            print(self.session_token)
            self.signin_result = True
        else:
            self.logger.error(f"签到失败，响应信息：{res.text}")
            self.signin_result = False
    def choujiang(self) -> None:
        """签到"""
        res = self.session.post(url=self.choujiang_url, headers={"Content-Type": "application/json"}, data=json.dumps({
            "item_id": 247
        }))
        self.signin_date = datetime.utcnow()
        if res.text.find("200") > -1:
            ...
            self.logger.error(f"抽奖失败，响应信息：{res.text}")
            self.signin_result = False
